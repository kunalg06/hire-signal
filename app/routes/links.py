...
def generate_link(assignment_id):
    ...
    for port_attempt in range(Config.DOCKER_PORT_RANGE_START, Config.DOCKER_PORT_RANGE_END):
        try:
            print(f"Attempt {port_attempt - Config.DOCKER_PORT_RANGE_START + 1}: Starting container for assignment {assignment_id} on port {port_attempt}")
